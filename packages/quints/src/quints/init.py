...
def _issuer_yaml(_answers: Answers) -> str:
    return "\n".join(
        [
            ...
            "bank:",
            "  CHF:",
            "    # The account every CHF invoice is paid into. With a regular IBAN the",
            "    # QR-bill carries a SCOR reference (RF…), which spells out the invoice",
            "    # number — readable on a bank statement and in a reminder email.",
            f"    iban: {_SAMPLE_CHF_IBAN}",
            f"    bic: {_SAMPLE_CHF_BIC}",
            f"    bank_name: {_SAMPLE_CHF_BANK}",
            "    # Paid by the numeric Swiss QR reference (QRR) instead? That needs a",
            "    # QR-IBAN and the six-digit identification your bank assigns you (UBS",
            "    # calls it the BESR-ID) — it has to be the first six digits of every QR",
            "    # reference. Uncomment all three, and keep `iban` for CHF from abroad:",
            "    # reference: qrr",
...
def _invoice_acme_yaml(answers: Answers) -> str:
    ...
    return "\n".join(
        [
            ...
            f"supply: Juli {year}",
            "customer: acme",
            "# The customer's own reference for this invoice — their PO number. It is",
            "# printed next to the invoice number and travels in the QR-bill's structured",
            "# billing information, so their accounts-payable software can match it.",
            f"customer_reference: PO-{year}-118",
            "items:",

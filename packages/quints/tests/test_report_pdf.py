...
def test_bilanz_lines_structure_and_swiss_numbers(tmp_path):
    ...
    totals = [l for l in lines if l["rule"]]
    assert [l["label"] for l in totals] == ["Total Aktiven", "Total Passiven"]
    assert totals[0]["amount"] == "1'170.00"  # Swiss apostrophe format for de
    detail = next(l for l in lines if l["code"] == "1020")
    assert detail["indent"] == 1 and detail["label"] == "Bankguthaben"

    english = report_pdf.bilanz_lines(bilanz, "en")
    assert [l["label"] for l in english if l["rule"]][0] == "Total assets"
    assert [l["amount"] for l in english if l["rule"]][0] == "1,170.00"


def test_erfolg_lines_flip_expense_signs(tmp_path):
    _, erfolg = _reports(tmp_path)
    lines = report_pdf.erfolg_lines(erfolg, "en")
    fee = next(l for l in lines if l["code"] == "6940")
...
def test_render_pdf_produces_file(tmp_path):
    ...
    out = report_pdf.render_pdf(bilanz, erfolg, "de", tmp_path / "statements.pdf",
                                issuer_path=Path("invoicing/issuer.yaml"))
